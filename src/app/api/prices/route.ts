...
export async function GET() {
  try {
    ...
    if (!res.ok) return NextResponse.json({ error: "行情服务暂不可用" }, { status: 503 });
    ...
    for (const line of text.trim().split("\n")) {
      ...
      dataMap.set(code, { name: fields[0], price, prevClose, open, high, low, change, changePct });
    }
    if (dataMap.size === 0) return NextResponse.json({ error: "行情解析失败，请稍后重试" }, { status: 503 });
    const prices = A_SHARE_STOCKS.map((s) => {
      ...
      return { symbol: s.symbol, marketSymbol: toMarketSymbol(s.symbol), name: d.name || s.name, price: d.price, prevClose: d.prevClose, change: Math.round(d.change * 100) / 100, changePct: Math.round(d.changePct * 100) / 100, open: d.open, high: d.high, low: d.low, source: "sina" };
    });
